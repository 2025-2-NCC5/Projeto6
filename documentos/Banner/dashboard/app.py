...
categorical_features = ['gender', 'contrycode', 'areacode', 'campaignName', 'response', 'sendAt_weekday_name']
...
# 🔹 Streamlit Dashboard
# ==========================
st.set_page_config(page_title="Dashboard GoCâmbio", layout="wide")
...
with aba1:
    ...
    with col1:
        ...
        fig_semana = px.bar(x=envios_semana.index, y=envios_semana.values,
    ...
    with col2:
        ...
        fig_dia = px.bar(x=envios_dia.index, y=envios_dia.values,
...
with aba2:
    ...
    fig_camp = px.bar(
        campanhas_scaled.melt(id_vars="campaignName",
    ...
    fig_genero = px.bar(
        genero_scaled.melt(id_vars="gender",
                           value_vars=['clientes_impactados', 'gasto_medio', 'taxa_impacto_%'],
                           var_name="Métrica", value_name="Valor"),
...
with aba3:
    ...
    fig_imp = px.bar(top_vars.sort_values(by="Importance", ascending=True),
